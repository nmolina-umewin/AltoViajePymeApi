...
module.exports = {
    all             : function(limit = 0, offset = 0) {
        return 'SELECT id FROM users WHERE deleted_at IS NULL ORDER BY id DESC, created_at DESC, updated_at DESC' + (limit ? ` LIMIT ${limit}` : '') + (offset ? ` OFFSET ${offset}` : '');
    },
    byCompany       : function(idCompany, limit = 0, offset = 0) {
        return `SELECT u.id FROM users AS u JOIN user_companies AS c ON c.id_user = u.id AND c.id_company = ${idCompany} WHERE u.deleted_at IS NULL ORDER BY u.id DESC, u.created_at DESC, u.updated_at DESC` + (limit ? ` LIMIT ${limit}` : '') + (offset ? ` OFFSET ${offset}` : '');
    },
